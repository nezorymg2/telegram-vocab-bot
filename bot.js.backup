...
bot.command('skip', async (ctx) => {
  ...
  if (session.step === 'smart_repeat_quiz' && session.smartRepeatStage === 1) {
    ...
    await ctx.reply('⏭️ Этап 1 (викторина) пропущен!\n\n🎧 Переходим к BBC 6 Minute English...');
    return await startBBCStage(ctx, session);
    
  } else if (session.step === 'bbc_listening' || session.step === 'bbc_quiz' || session.step === 'bbc_completion') {
    // Пропускаем BBC этап, переходим к этапу 2
    await ctx.reply('⏭️ BBC этап пропущен!\n\n🧠 <b>Умное повторение - Этап 2/4</b>\n<b>Знаю/Не знаю</b>\n\nПереходим к быстрой оценке слов...');
    return await continueToBBCCompletion(ctx, session);
...
bot.on('message:text', async (ctx) => {
  ...
  // ===== BBC 6 MINUTE ENGLISH HANDLERS =====
  
  // Handle BBC listening stage
  if (session.step === 'bbc_listening') {
    if (text === '✅ Прослушал') {
      return await startBBCQuiz(ctx, session);
    }
    
    if (text === '⏭️ Пропустить BBC') {
      await ctx.reply('⏭️ BBC этап пропущен. Переходим к следующему этапу...');
      return await continueToBBCCompletion(ctx, session);
    }
    ...
    return ctx.reply('Пожалуйста, выберите один из вариантов:', {
      reply_markup: new Keyboard()
        .text('✅ Прослушал')
        .text('⏭️ Пропустить BBC').row()
        .resized()
    });
  }
  
  // Handle BBC quiz answers
  if (session.step === 'bbc_quiz') {
    return await processBBCAnswer(ctx, session, text);
  }
  
  // Handle BBC completion stage
  if (session.step === 'bbc_completion') {
    if (text === '🔄 Переслушать для закрепления') {
      const bbcData = session.bbcStage;
      if (bbcData && bbcData.mp3Url) {
        try {
          await ctx.replyWithAudio(bbcData.mp3Url, {
            caption: `🔄 ${bbcData.title}`
          });
        } catch (error) {
          console.error('Error replaying BBC audio:', error);
          await ctx.reply('⚠️ Не удалось воспроизвести аудио повторно.');
        }
      }
      
      // Show continue button again
      const keyboard = new Keyboard()
        .text('🔄 Переслушать для закрепления')
        .text('➡️ Продолжить').row()
        .resized();
      
      return ctx.reply('🎧 Аудио воспроизведено повторно. Готовы продолжить?', {
...
async function handleSmartRepeatQuizAnswer(ctx, session, answerText) {
  ...
  if (quizSession.currentQuestionIndex >= quizSession.words.length) {
    ...
    // Потом переходим к BBC этапу
    await startBBCStage(ctx, session);
...
// ===== BBC 6 MINUTE ENGLISH FUNCTIONS =====

// Start BBC stage between stage 1 and stage 2 of smart repeat
async function startBBCStage(ctx, session) {
  try {
    console.log('=== BBC STAGE START ===');
    
    const userId = ctx.from.id.toString();
    const profileName = session.profile;
    
    // Get next unfinished BBC episode
    const episode = await bbcService.getNextBBCEpisode(userId, profileName);
    
    if (!episode) {
      await ctx.reply('📻 Все доступные эпизоды BBC 6 Minute English пройдены!\n\n➡️ Переходим к следующему этапу...');
      return await startSmartRepeatStage2(ctx, session);
    }
    
    // Store episode info in session
    session.bbcStage = {
      ...
      currentQuestionIndex: 0,
      answers: [],
      // Store full episode data for generateBBCQuestions
      fullEpisodeData: episode
    };
    
    session.step = 'bbc_listening';
    
    // Send audio and episode info
    const message = `🎧 <b>BBC 6 Minute English</b>\n\n` +
      `📻 <b>Эпизод:</b> ${episode.title}\n\n` +
      `🔊 Прослушайте аудио и нажмите "Прослушал", когда будете готовы ответить на вопросы.`;
    ...
    if (episode.mp3Url) {
      try {
        await ctx.replyWithAudio(episode.mp3Url, {
          caption: `🎧 ${episode.title}`
        });
      } catch (error) {
        console.error('Error sending BBC audio:', error);
        await ctx.reply('⚠️ Не удалось загрузить аудио. Можете прослушать на сайте BBC.');
      }
    }
    
  } catch (error) {
    console.error('Error starting BBC stage:', error);
    await ctx.reply('❌ Ошибка при загрузке BBC эпизода. Переходим к следующему этапу...');
    return await startSmartRepeatStage2(ctx, session);
  }
}

// Handle "Прослушал" button and start quiz
async function startBBCQuiz(ctx, session) {
  try {
    console.log('=== BBC QUIZ START ===');
    
    const episodeData = session.bbcStage;
    if (!episodeData) {
      await ctx.reply('❌ Ошибка: данные эпизода не найдены');
      return await startSmartRepeatStage2(ctx, session);
    }
    
    console.log('DEBUG: Episode data:', {
      title: episodeData.title,
      pageUrl: episodeData.pageUrl,
      hasTranscript: !!episodeData.transcript
    });
    
    // Generate questions for this episode
    await ctx.reply('🤔 Генерирую вопросы по эпизоду...');
    
    const bbcData = await bbcService.generateBBCQuestions(episodeData.fullEpisodeData || {
      title: episodeData.title,
      pageUrl: episodeData.pageUrl,
      transcript: episodeData.transcript
    });

    console.log('DEBUG: BBC Data received:', {
      hasQuestions: !!(bbcData && bbcData.questions),
      questionsLength: bbcData && bbcData.questions ? bbcData.questions.length : 0,
      enhanced: bbcData && bbcData.enhanced,
      hasVocabulary: !!(bbcData && bbcData.vocabulary && bbcData.vocabulary.length > 0),
      hasTranscript: !!(bbcData && bbcData.transcript),
      fullStructure: bbcData ? Object.keys(bbcData) : 'no bbcData'
    });

    // Handle new structure with enhanced mode
    const questions = bbcData.questions || [];
    
    if (!questions || questions.length === 0) {
      // If no questions, skip directly to next stage
      await ctx.reply('❌ Не удалось сгенерировать вопросы по эпизоду. Переходим к следующему этапу...');
      return await startSmartRepeatStage2(ctx, session);
    }

    session.bbcStage.questions = questions;
    session.step = 'bbc_quiz';
    
    await sendBBCQuestion(ctx, session);
    
  } catch (error) {
    console.error('Error starting BBC quiz:', error);
    await ctx.reply('❌ Ошибка при создании викторины. Переходим к следующему этапу...');
    return await startSmartRepeatStage2(ctx, session);
  }
}

// Send current BBC question
async function sendBBCQuestion(ctx, session) {
  try {
    const bbcData = session.bbcStage;
    const currentIndex = bbcData.currentQuestionIndex;
    const question = bbcData.questions[currentIndex];
    
    if (!question) {
      return await finishBBCQuiz(ctx, session);
    }
    
    const totalQuestions = bbcData.questions.length;
    const progressText = `📊 <b>Вопрос ${currentIndex + 1}/${totalQuestions}</b>\n\n`;
    
    if (question.type === 'multiple_choice') {
      // TEMPORARY: Show options as text instead of buttons to debug
      let optionsText = '\n';
      question.options.forEach((option, index) => {
        ...
        if (typeof option === 'string') {
          ...
          optionText = option.text || option.value || option.answer || JSON.stringify(option);
        } else {
          optionText = `Option ${index + 1}`;
        }
        optionsText += `${String.fromCharCode(97 + index)}) ${optionText}\n`;
      });
      
      await ctx.reply(progressText + question.question + optionsText + '\n💡 <i>Выберите правильный вариант (a, b, c, d)</i>', {
        reply_markup: { remove_keyboard: true },
        parse_mode: 'HTML'
      });
    } else if (question.type === 'short_answer') {
      await ctx.reply(progressText + question.question + '\n\n💡 <i>Напишите ваш ответ (1-3 слова)</i>', {
        reply_markup: { remove_keyboard: true },
        parse_mode: 'HTML'
      });
    }
    ...
    console.error('Error sending BBC question:', error);
    await finishBBCQuiz(ctx, session);
  }
}

// Process BBC answer and move to next question
async function processBBCAnswer(ctx, session, userAnswer) {
  try {
    const bbcData = session.bbcStage;
    const currentIndex = bbcData.currentQuestionIndex;
    const question = bbcData.questions[currentIndex];
    ...
    const isCorrect = bbcService.checkBBCAnswer(question, userAnswer);
    
    // Store the answer
    bbcData.answers.push({
    ...
    bbcData.currentQuestionIndex++;
    
    // Send immediate feedback
    const feedback = isCorrect ? '✅ Правильно!' : '❌ Неправильно';
    await ctx.reply(feedback);
    
    // Check if quiz is finished
    if (bbcData.currentQuestionIndex >= bbcData.questions.length) {
      return await finishBBCQuiz(ctx, session);
    } else {
      // Send next question
      return await sendBBCQuestion(ctx, session);
    }
    
  } catch (error) {
    console.error('Error processing BBC answer:', error);
    return await finishBBCQuiz(ctx, session);
  }
}

// Finish BBC quiz and show results
async function finishBBCQuiz(ctx, session) {
  try {
    console.log('=== BBC QUIZ FINISH ===');
    
    const bbcData = session.bbcStage;
    const correctCount = bbcData.answers.filter(a => a.correct).length;
    const totalQuestions = bbcData.questions.length;
    const score = Math.round((correctCount / totalQuestions) * 100);
    
    // Show results
    let resultsMessage = `🏆 <b>BBC Quiz завершен!</b>\n\n` +
      `📊 <b>Результат:</b> ${correctCount}/${totalQuestions} (${score}%)\n\n` +
      `<b>Правильные ответы:</b>\n`;
    
    bbcData.questions = bbcData.questions || [];
    bbcData.questions.forEach((question, index) => {
      const userAnswer = bbcData.answers[index];
      const isCorrect = userAnswer && userAnswer.correct;
      const icon = isCorrect ? '✅' : '❌';
      
      resultsMessage += `${icon} <b>Вопрос ${index + 1}:</b> ${question.question}\n`;
      resultsMessage += `<b>Правильный ответ:</b> ${question.correct_answer}\n`;
      if (question.explanation) {
        resultsMessage += `<i>${question.explanation}</i>\n`;
      }
      resultsMessage += '\n';
    });
    ...
    const userId = ctx.from.id.toString();
    const profileName = session.profile;
    await bbcService.saveBBCProgress(userId, profileName, bbcData.episodeId, score, bbcData.answers);
    
    // Show transcript and vocabulary
    await sendBBCTranscript(ctx, session);
    
  } catch (error) {
    console.error('Error finishing BBC quiz:', error);
    await continueToBBCCompletion(ctx, session);
  }
}

// Send transcript and vocabulary
async function sendBBCTranscript(ctx, session) {
  try {
    const bbcData = session.bbcStage;
    
    // Send transcript in chunks if too long
    const transcriptChunks = bbcService.formatTranscript(bbcData.transcript);
    
    await ctx.reply('📜 <b>Транскрипт эпизода:</b>', { parse_mode: 'HTML' });
    
    for (let i = 0; i < transcriptChunks.length; i++) {
      const chunk = transcriptChunks[i];
      const header = transcriptChunks.length > 1 ? `<b>Часть ${i + 1}/${transcriptChunks.length}:</b>\n\n` : '';
      
      await ctx.reply(header + chunk, { parse_mode: 'HTML' });
    ...
    // Vocabulary sending is disabled
    // if (bbcData.vocabulary && bbcData.vocabulary.length > 0) {
    //   let vocabMessage = '📚 <b>Ключевые слова:</b>\n\n';
    //   
    //   bbcData.vocabulary.forEach(item => {
    //     vocabMessage += `🔤 <b>${item.term}</b>`;
    //     if (item.translationRu) {
    //       vocabMessage += ` - ${item.translationRu}`;
    //     }
    //     if (item.definition) {
    //       vocabMessage += `\n<i>${item.definition}</i>`;
    //     }
    ...
    // Offer to replay audio
    const keyboard = new Keyboard()
      .text('🔄 Переслушать для закрепления')
      .text('➡️ Продолжить').row()
      .resized();
    
    await ctx.reply('🎧 Хотите переслушать аудио для закрепления?', {
      reply_markup: keyboard
    });
    
    session.step = 'bbc_completion';
    
  } catch (error) {
    console.error('Error sending BBC transcript:', error);
    await continueToBBCCompletion(ctx, session);
  }
}

// Handle completion and transition to stage 2
async function continueToBBCCompletion(ctx, session) {
  try {
    // Clean up BBC session data
    delete session.bbcStage;
    
    // Transition to stage 2
    session.step = 'smart_repeat_stage_2';
    session.smartRepeatStage = 2;
    
    await ctx.reply('✅ <b>BBC этап завершен!</b>\n\n🧠 <b>Умное повторение - Этап 2/4</b>\n<b>Знаю/Не знаю</b>\n\nПереходим к быстрой оценке слов...', {
      parse_mode: 'HTML'
    });
    ...
    console.error('Error in BBC completion:', error);
    session.step = 'main_menu';
